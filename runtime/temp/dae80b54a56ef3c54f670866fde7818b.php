<?php /*a:1:{s:44:"E:\tp\tp\app\admin\view\admin\user-list.html";i:1552465532;}*/ ?>
  ...
  <body>
    ...
    <div class="x-body">
      ...
      <xblock>
        ...
        <!--<span class="x-right" style="line-height:40px">共有数据：<?php echo htmlentities($user_list['total']); ?> 条</span>-->
      </xblock>
      <table class="layui-table x-admin">
        ...
        <?php if(is_array($users) || $users instanceof \think\Collection || $users instanceof \think\Paginator): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "无数据" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?>
            <tr>
                ...
                <td class="td-manage">
                    <?php if($user['is_use'] == 1): ?>
                        <a onclick="switch_status(this,'<?php echo htmlentities($user['id']); ?>',0)" title="停用" href="javascript:;">
                            <i class="layui-icon">&#xe601;</i>
                        </a>
                    <?php else: ?>
                        <a onclick="switch_status(this,'<?php echo htmlentities($user['id']); ?>',1)" href="javascript:;" title="启用">
                            <i class="layui-icon">&#xe62f;</i>
                        </a>
                    <?php endif; ?>

                    <a title="编辑"  onclick="x_admin_show('编辑','<?php echo htmlentities($update_user_url); ?>/id/<?php echo htmlentities($user['id']); ?>',950,600)">
                        <i class="layui-icon">&#xe642;</i>
                    </a>
                    <!--<a onclick="x_admin_show('修改密码','member-password.html',600,400)" title="修改密码" href="javascript:;">
                        <i class="layui-icon">&#xe631;</i>
                    </a>-->
                    <a title="删除" onclick="member_del(this,'<?php echo htmlentities($user['id']); ?>')" href="javascript:;">
                        <i class="layui-icon">&#xe640;</i>
                    </a>
                </td>
            </tr>
...
console.log(111)
              if($(obj).attr('title')=='停用'){
                  $.post('updateuser',{'id':id,'is_use':0},function(res){
                        //发异步把用户状态进行更改
                      if (res.status) {
                          $(obj).attr('title','启用')
                          $(obj).find('i').html('&#xe62f;');
                  ...
                  $.post('updateuser',{'id':id,'is_use':1},function(res){
                      //发异步把用户状态进行更改
                      if (res.status) {
                          $(obj).attr('title','停用')
                          $(obj).find('i').html('&#xe601;');
      ...
      function member_del(obj,id){
          layer.confirm('确认要删除吗？',function(index){
              //发异步删除数据
              $.post('updateuser',{"id":id,"status":0},function(res){
                  if (res.status) {
      ...
      function delAll (argument) {
        ...
        layer.confirm('确认要删除吗？',function(index){
            $.post('delAll',{'id':data},function(res){
                if (res.status) {
                    //捉到所有被选中的，发异步进行删除
